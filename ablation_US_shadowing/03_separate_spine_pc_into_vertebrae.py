...
def load_centered_spine_pcd(root_path_spine, spine_id, deform):
    ...
    name = namings.get_name_spine_lumbar_mesh_deformed_scaled_centered(spine_id,deform).split(".")[0]
    path_rendered_pcd = os.path.join(root_path_spine,spine_id,"rendering")
    path_spine = os.path.join(path_rendered_pcd,name + ".pcd")
    print("Loading" + str(path_spine))
    spine_pcd = o3d.io.read_point_cloud(path_spine)
    return spine_pcd,path_rendered_pcd
...
def separate_US_pointcloud_into_vertebrae(root_path_spine, root_path_vertebrae, spine_id, deform, visualize=False):
    ...
    spine_pcd,path_rendered = load_centered_spine_pcd(root_path_spine, spine_id, deform)
    ...
    for vert_ind in range(5):
        ...
        to_save_dir = os.path.join(root_folder_vert,"render")
