...
def processAllVertebrae(list_path, rootDirectoryVertebrae, saveTo,
    ...
    for model_id in model_list:
        ...
        pcd_root = os.path.join(rootDirectoryVertebrae, vert_folder_name, "render")

        for deform in range(nr_deform_per_sample):

            logging.debug(str(idx) + "/" + str(len(model_list) * nr_deform_per_sample))
            logging.debug("Processing " + str(model_id) + " deformation: " + str(deform) )

            # get the name of the pcd and the name of the mesh
            polluted_pcd_path = os.path.join(pcd_root,
