...
if __name__ == '__main__':
    ...
    if 'center_mesh' in pipeline or 'all' in pipeline:
        subprocess.run(['python', '01_center_spines_and_vertebrae.py',
    ...
    if 'scale_down_mesh' in pipeline or 'all' in pipeline:
        misc.create_list_all_vert_and_spines_from_spineid(list_spines, list_paths_spines_and_vert_for_scaling,
                                                                   root_paths_vertebrae, root_paths_spines)
        subprocess.run(['python', '02_scale_down_mesh.py',
                        '--list_mesh_paths', list_paths_spines_and_vert_for_scaling])

    if 'shift_and_merge' in pipeline or 'all' in pipeline:
        misc.create_list_all_scaled_spines_from_spineid(list_spines, list_paths_spines, root_paths_spines)
        subprocess.run(['python', '03_shift_and_merge_spine.py',
                        '--list_paths_spine', list_paths_spines,
                        '--num_shifts',num_shifts])

    if 'get_camera_poses' in pipeline or 'all' in pipeline:
        misc.create_list_all_scaled_vert_and_spines_from_spineid(list_spines,
                                                                          list_paths_spines_and_vert_for_camera_poses_generation,
                                                                          root_paths_vertebrae, root_paths_spines)
        subprocess.run(['python', '04_generate_camera_poses.py',
    ...
    if 'raycast' in pipeline or 'all' in pipeline:
        ...
        misc.create_list_init_shifted_merged_for_raycasting(spines_root=root_paths_spines,
                                                                     list_spines=list_spines,
                                                                     save_to=list_paths_for_raycasting,
                                                                     num_shifts=num_shifts)
        subprocess.run(['python', '05_generate_partial_pointclouds_from_spine.py',
    ...
    if 'account_US_shadows' in pipeline or 'all' in pipeline:
        subprocess.run(['python', '06_accounts_US_shadows.py',
                        '--root_paths_spines', root_paths_spines,
                        '--list_spines', list_spines,
                        ])

    if 'separate_spine_pc_into_vert' in pipeline or 'all' in pipeline:
        subprocess.run(['python', '07_separate_spine_pc_into_vertebrae.py',
                        '--list_file_names', list_spines,
                        '--root_path_vertebrae', root_paths_vertebrae,
                        '--root_path_spines', root_paths_spines,
    ...
    if 'create_h5_dataset' in pipeline or 'all' in pipeline:
        ...
        subprocess.run(['python', '08_get_list_vertebrae_in_folders.py',
                        '--root_path_vertebrae', root_paths_vertebrae,
        ...
        subprocess.run(['python', '08_create_dataset_for_shape_completion.py',
