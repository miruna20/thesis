...
def load_centered_spine_pcd(root_path_spine, spine_id, shift):

    path_root_shifts = os.path.join(root_path_spine,spine_id,"shifts", namings.get_name_spine_lumbar_mesh_scaled_centered(spine_id).split(".")[0])
...
def load_centered_scaled_vertebrae(root_path_vertebrae, spine_id):
    paths_vertebrae = namings.get_paths_vertebrae_scaled(root_path_vertebrae,spine_id)
...
def load_pcd(root_path_spine, spine_id):
    look_for = "**/*" + str(spine_id) + '*.pcd'
    path_pcd = glob.glob(os.path.join(root_path_spine, look_for), recursive=True)

    if (len(path_pcd) != 1):
        print("More than 1 or no pcd found for spine " + str(spine_id),file=sys.stderr)

    pcd = o3d.io.read_point_cloud(path_pcd[0])

    return pcd

def separate_US_pointcloud_into_vertebrae(root_path_spine, root_path_vertebrae, spine_id, shift, visualize=False):
    print("Separating US point cloud from: " + str(spine_id))

    #  load the centered, scaled spine pcd
    spine_pcd, path_shifts = load_centered_spine_pcd(root_path_spine, spine_id, shift)

    #  load vertebrae that are also scaled and centered
    vertebrae_meshes, vertebrae_paths = load_centered_scaled_vertebrae(root_path_vertebrae, spine_id)
...
if __name__ == "__main__":
    ...
    for spine_id in spine_ids:
        for shift in range(int(args.nr_shift_per_spine)):
            separate_US_pointcloud_into_vertebrae(args.root_path_spines, args.root_path_vertebrae, spine_id, shift, visualize=args.visualize)