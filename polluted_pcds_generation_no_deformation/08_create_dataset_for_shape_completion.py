...
def processAllVertebrae(list_path, rootDirectoryVertebrae, saveTo,
                        nr_shifts_per_sample, visualize=False, nrPointsProPartialPC=2048,
                        nrPointsProCompletePC=4096):
    ...
    for model_id in model_list:
        ...
        for shift in range(int(nr_shifts_per_sample)):
            ...
            polluted_pcd_path = os.path.join(rootDirectoryVertebrae, vert_folder_name, "shifts",
                                             shift_folders[shift],
                                             namings.get_name_undeformed_polluted_vert_pcd(vert_folder_name))
            vert_mesh_path = os.path.join(rootDirectoryVertebrae, vert_folder_name,
                                          namings.get_name_undeformed_vert_deform_scaled(vert_folder_name))
            ...
            dataset_ids.append((model_id + "_shift" + str(shift)).encode("ascii"))
...
if __name__ == "__main__":
    ...
    logging.basicConfig(filename="create_dataset_logs" + timestr + ".txt", filemode='w',level=logging.DEBUG, force=True)
