...
def shadow_one_x_side(pcd_init, pcd_merged, pos=True):
    ...
    pcd_init_and_merged.points = o3d.utility.Vector3dVector(points_from_all_pcds_stacked)
...
def delete_shadows_from_pcd(pcd_init, pcd_merged_posx, pcd_merged_negx, visualize=False):
    ...
    shadowed_pcd_posx = shadow_one_x_side(pcd_init, pcd_merged_posx, pos=True)
    shadowed_pcd_negx = shadow_one_x_side(pcd_init, pcd_merged_negx, pos=False)
    
    shadowed_pcd = shadowed_pcd_posx.extend(shadowed_pcd_negx)
...
def delete_shadows_from_pcd(pcd0_path, pcd_combined_path, save_path, visualize=False):
        ...
        for deform in range(int(args.num_deform)):
            ...
            for shift_dir in sorted(os.listdir(shifts_root)):
               ...
               delete_shadows_from_pcd(pcd_init=os.path.join(rendering_path,path_to_init_pcds[deform]),
                                    pcd_merged_posx=os.path.join(curr_rendering,name + "_posxmerged.pcd"),
                                    pcd_merged_negx=os.path.join(curr_rendering,name + "_negxmerged.pcd"),
                                    save_path=os.path.join(os.path.join(shifts_root,shift_dir,"account_for_shadow.pcd"))
                """
