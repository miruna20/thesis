...
def processOneVertebra(pathCompleteVertebra, pathToPartialPCD, nrPointsProPartialPC=2048,
    ...
    if (nr_points_in_partial_pcd >= nrPointsProPartialPC):
        ...
        logging.debug("Using FPS: " + str(sampled_partial_pcd.shape[0]))
    else:
    ...
    for model_id in model_list:
        ...
        for shift in range(int(nr_shifts_per_sample)):
            for deform in range(nr_deform_per_sample):
                ...
                unpolluted_pcd_path = os.path.join(rootDirectoryVertebrae, vert_folder_name, "shifts", shift_folders[shift],
                                                 namings.get_name_unpolluted_vert_pcd(vert_folder_name, deform))
...
if __name__ == "__main__":
    ...
    addname = ""
    if "train" in args.folder_complete_meshes:
        addname = "train"
    elif "valid" in args.folder_complete_meshes:
        addname = "valid"
    elif "test" in args.folder_complete_meshes:
        addname = "test"

    logging.basicConfig(filename="create_dataset_logs_" +  addname + "_" + timestr + ".txt", filemode='w', level=logging.DEBUG,
                        force=True)
