...
def crop_vert(data, centroid, crop_size=(0, 128, 128)):
    ...
    # Pad the array
    cropped_data = np.pad(cropped_data, ((0, 0), (pad_before[1], pad_after[1]), (pad_before[2], pad_after[2])),
                          mode='constant')

    return cropped_data


def generate_2D_labelmap(image, savePath, spine_id, level):
    print("Generating labelmap")

    vert_segm_np_data = image.get_fdata()
    ...
    vert_image_2D = nib.Nifti1Image(labelmap_2D_in_3D, image.affine, image.header)
        ...
        if (level in numpyData):
            ...
            if (not os.path.exists(Path(savePath))):
                ...
                if (generate2DLabelmap):
                    image = nib.load(path_segm_vert)
                    generate_2D_labelmap(image, savePath, spine_id, level)
                continue
            ...
            # Reorient to RAI
            vertImage_reoriented = nib.funcs.as_closest_canonical(vertImage)
            nib.save(vertImage_reoriented, path_segm_vert)

            # Generate 2D labelmaps that simulate Xray annotations
            if (generate2DLabelmap):
                generate_2D_labelmap(vertImage_reoriented, savePath, spine_id, level)
...
if __name__ == '__main__':
    ...
    arg_parser = argparse.ArgumentParser(
        description="Separate spine segmentations into segmentations of individual vertebrae")
    ...
    separate_spines_into_vertebrae(args.txt_file, args.root_path_spines, args.root_path_vertebrae,
                                   args.generate2DLabelmap)
