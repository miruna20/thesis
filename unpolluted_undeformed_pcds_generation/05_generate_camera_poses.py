...
def create_csv_with_camera_poses(list_path_spines_and_vert, camera_poses_csv_path,num_deform):
    ...
    for i in range(0,int(len(spines_and_vert)/6)):
        ...
        if(num_deform != str(0)):
            ...
            name_spine_posxmerged = namings.get_name_spine_lumbar_mesh_scaled_centered_posxmerged(spine)
            csv_file.write(create_row(name_spine_posxmerged.split(".")[0], camera_poses))

            # creating entry for the negx merged spine
            name_spine_negxmerged = namings.get_name_spine_lumbar_mesh_scaled_centered_negxmerged(spine)
            csv_file.write(create_row(name_spine_negxmerged.split(".")[0], camera_poses))
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument(
        ...
        required=True,
        dest="num_deform",
        help="number of deformations"
    )

    args = arg_parser.parse_args()
    print("Generating camera poses")

    create_csv_with_camera_poses(args.list_spines_and_corresp_vertebrae, args.csv_file,args.num_deform)