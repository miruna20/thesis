...
if __name__ == "__main__":
    ...
    if(args.nr_deform_per_spine != str(0)):
        for spine_id in spine_ids:
            try:
                for deform in range(int(args.nr_deform_per_spine)):
                    for shift in range(int(args.nr_shift_per_spine)):
                        separate_US_pointcloud_into_vertebrae(args.root_path_spines, args.root_path_vertebrae, spine_id,
                                                              deform, shift, visualize=args.visualize)
            except:
        ...
        for spine_id in spine_ids:
            try:
                for shift in range(int(args.nr_shift_per_spine)):
                        separate_US_pointcloud_into_vertebrae(args.root_path_spines, args.root_path_vertebrae, spine_id,
                                                              shift, visualize=args.visualize)
            except:
                print("Something went wrong with processing" + str(spine_id))
