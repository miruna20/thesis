        ...
        for spine in spines_list:
            ...
            for shift in range(int(num_shifts)):
                ...
                merged_spine_posx = os.path.join(shifts_path,
                                                 ...
                                                 namings.get_name_spine_lumbar_mesh_scaled_centered_posxmerged(
                                                 spine))

                # add merged_spine_negx
                merged_spine_negx = os.path.join(shifts_path,
                                                 shifts_directories[shift],
                                                 namings.get_name_spine_lumbar_mesh_scaled_centered_negxmerged(
                                                     spine))
...
def create_list_all_deformed_scaled_spines_from_spineid(list_of_spine_ids, save_to, root_spines, num_deform):
    ...
    if (num_deform == str(0)):
        for spine_id in spine_ids:
            ...
            paths_spine = namings.get_path_centered_scaled_lumbar_spine(root_spines, spine_id)
            print(paths_spine)
            print("\n")
...
def create_list_all_deformed_vert_and_spines_from_spineid(list_of_spine_ids, save_to, root_vert, root_spines,
    ...
    if(num_deform == str(0)):
        for spine_id in spine_ids:
            # find the paths of all deformed spines
            paths_spine = namings.get_path_lumbar_spine_centered(root_spines, spine_id)
            paths_vertebrae = namings.get_paths_vertebrae_centered(root_vert, spine_id)
            print(paths_spine)
            print("\n")
            list.write(paths_spine + "\n")
